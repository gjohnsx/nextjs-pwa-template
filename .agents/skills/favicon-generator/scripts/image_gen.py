...
def _auto_load_local_env_files() -> None:
    loaded_from: List[str] = []
    for filename in (".env.local", ".env"):
        path = Path.cwd() / filename
        loaded_count = _load_env_file(path)
        if loaded_count > 0:
            loaded_from.append(f"{path.name} ({loaded_count})")
    if loaded_from:
        print(f"Loaded env vars from: {', '.join(loaded_from)}", file=sys.stderr)
